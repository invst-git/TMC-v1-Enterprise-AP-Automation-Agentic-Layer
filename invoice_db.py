...
def find_duplicate_invoice_candidates(
    ...
    *,
    ...
    if not vendor_id and not normalized_supplier_name:
        return []
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            if vendor_id and normalized_supplier_name:
                ...
                where.append("lower(coalesce(i.supplier_name,'')) = lower(%s)")
                params.append(normalized_supplier_name)
            ...
            for row in cur.fetchall():
                matched_invoice_date = row[2]
                matched_total_amount = float(row[3]) if row[3] is not None else None
                signals = ["same_invoice_number"]
                confidence = 0.55

                if vendor_id and row[5] and str(row[5]) == str(vendor_id):
                    signals.append("same_vendor")
                    confidence += 0.20
                elif normalized_supplier_name and row[6] and row[6].strip().lower() == normalized_supplier_name.lower():
                    signals.append("same_supplier_name")
                    confidence += 0.15

                if total_amount is not None and matched_total_amount is not None:
                    if abs(float(total_amount) - matched_total_amount) <= 0.01:
                        signals.append("same_total_amount")
                        confidence += 0.15

                if parsed_invoice_date and matched_invoice_date and matched_invoice_date == parsed_invoice_date:
                    signals.append("same_invoice_date")
                    confidence += 0.10
                ...
                candidates.append(
                    {
                        ...
                        "invoiceDate": matched_invoice_date.isoformat() if matched_invoice_date else None,
                        "totalAmount": matched_total_amount,
                        "status": row[4] or "",
                        "vendorId": str(row[5]) if row[5] else None,
                        "supplierName": row[6] or "",
                        "signals": signals,
...
def update_invoice_matching_outcome(
    invoice_id: str,
    *,
    ...
    if status is None and matched_po_id is None and confidence is None:
        return False
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                SET
                  ...
                  matched_po_id = COALESCE(%s, matched_po_id),
                  confidence = COALESCE(%s, confidence)
                WHERE id = %s
                """,
                (status, matched_po_id, confidence, invoice_id),
            )
